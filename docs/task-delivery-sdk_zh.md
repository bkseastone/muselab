...
点击聊天工具栏 Activity 旁的「任务交付与环境」，选择任务，查看实际记录的文件、执行命令、消息证据、Git 差异和文件检查点。非 Git 工作目录也能使用文件与命令证据。
...
环境面板显示已登记的工作目录、分支／worktree、dirty 状态、后端和主机；服务来源是当前已鉴权页面的 origin。预览与终端使用各自的工作目录，可能与当前聊天不同。终端标签表示实际进程所在目录，不声称终端经过模型后端。自动 Git 探测缓存 5 秒，打开或刷新交付面板会重新探测。Git 探测只读，单条命令超时 2 秒并限制输出；不会执行 Git 配置中的文件系统监视命令。

## 如何审阅文件恢复

1. 结束或停止任务，清空待发队列，并删除原生定时任务。同一工作区内的其他 MuseLab 会话也必须空闲。
