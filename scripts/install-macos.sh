...
# dotenv and systemd EnvironmentFile both accept double-quoted values. Escape
# the two characters that are special inside those quotes so checkout paths
# containing spaces, backslashes, or double quotes remain a single value.
SESSIONS_DIR_ENV="${SESSIONS_DIR//\\/\\\\}"
SESSIONS_DIR_ENV="${SESSIONS_DIR_ENV//\"/\\\"}"
if [[ -f .env ]]; then
  ok ".env already exists — preserving all existing values"
else
  ...
  echo
  echo "  Primary workspace = the directory muselab can read and modify."
  echo "  主工作区 = muselab 可以读取和修改的目录。"
  echo "  Use a dedicated project/data directory; NEVER point it at \$HOME or /."
  echo "  请选择专用的项目或资料目录；绝对不要指向 \$HOME 或 /。"
  WORKSPACE="$(ask 'Primary workspace / 主工作区 (absolute path / 绝对路径):' "$HOME/muselab-workspace")"
  WORKSPACE="${WORKSPACE/#\~/$HOME}"
  if ! mkdir -p "$WORKSPACE" 2>/dev/null; then
    err "cannot create $WORKSPACE (permission denied?). Pick a path under your home."
    exit 1
  fi
  if ! ( touch "$WORKSPACE/.muselab-write-test" && rm -f "$WORKSPACE/.muselab-write-test" ) 2>/dev/null; then
    err "$WORKSPACE exists but isn't writable. Run: chmod u+rwx $WORKSPACE"
    exit 1
  fi
...
MUSELAB_TOKEN=$TOKEN
MUSELAB_ROOT=$WORKSPACE
# Bind durable session metadata to this installation's actual checkout path.
MUSELAB_SESSIONS_DIR="$SESSIONS_DIR_ENV"
...
EOF
  ...
  ok "  MUSELAB_SESSIONS_DIR = $SESSIONS_DIR"
  ok "  MUSELAB_TOKEN saved in .env (value not displayed)"
fi

# Upgrade existing installations without replacing an explicit custom path.
# A commented example is not an active assignment, so it still gets the
# stable checkout-local default appended below.
if ! grep -Eq '^[[:space:]]*MUSELAB_SESSIONS_DIR[[:space:]]*=' .env; then
  {
    printf '\n# Added by install-macos.sh: stable session metadata path\n'
    printf 'MUSELAB_SESSIONS_DIR="%s"\n' "$SESSIONS_DIR_ENV"
  } >> .env
  ok "  MUSELAB_SESSIONS_DIR added = $SESSIONS_DIR"
fi

# ----- 4. LaunchAgents ---------------------------------------------------
PORT="$(grep -E '^MUSELAB_PORT=' .env 2>/dev/null | head -1 | cut -d= -f2 | tr -d '[:space:]')"
PORT="${PORT:-8765}"
PRIMARY_WORKSPACE="$(grep -E '^MUSELAB_ROOT=' .env 2>/dev/null | head -1 | cut -d= -f2-)"
PRIMARY_WORKSPACE="${PRIMARY_WORKSPACE#"${PRIMARY_WORKSPACE%%[![:space:]]*}"}"
PRIMARY_WORKSPACE="${PRIMARY_WORKSPACE%"${PRIMARY_WORKSPACE##*[![:space:]]}"}"
if (( ${#PRIMARY_WORKSPACE} >= 2 )) \
    && [[ "${PRIMARY_WORKSPACE:0:1}" == '"' && "${PRIMARY_WORKSPACE: -1}" == '"' ]]; then
  PRIMARY_WORKSPACE="${PRIMARY_WORKSPACE:1:${#PRIMARY_WORKSPACE}-2}"
fi
...
if [[ -n "$PRIMARY_WORKSPACE" ]]; then
  echo  "  Primary workspace / 主工作区: $PRIMARY_WORKSPACE"
fi
echo
...
if [[ -n "$PRIMARY_WORKSPACE" && ! -f "$PRIMARY_WORKSPACE/CLAUDE.md" ]]; then
  echo  "  Optional workspace instructions / 可选工作区说明:"
  echo  "    bash scripts/intake.sh"
  echo  "  This creates CLAUDE.md only; it does not create a fixed folder structure."
  echo  "  该命令只创建 CLAUDE.md，不会创建固定目录结构。"
  echo
fi
